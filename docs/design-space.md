...
| Binary KV + B+ tree + standalone | Common persistent semantics + structural amplification evidence implemented | `db-storage-btree` implements the full 4 KiB-key/1 MiB-value point/range contract, deterministic fault recovery, and the common amplification-report trait. Hand-computable traces measure logical validated-page accesses (including cache hits), synchronized data-page writes, and retained committed data-page space including reusable COW history. The shared Phase 4 runner now drives this evidence from versioned common traces and gates reports on step-by-step logical agreement with the LSM. Read work is explicitly tagged `btree_page_access`; it is not presented as device I/O. Physical compaction and controlled-host measurements remain deferred |
| Binary KV + LSM + standalone | Persistent correctness + common structural amplification evidence; not yet a timing-performance candidate | `db-storage-lsm` implements the common point/reopen/range contract, crash-safe WAL/SSTable/Manifest-v5 full-set compaction, deterministic old-or-new fault recovery, and multi-cycle differential compaction tests. Its proven raw counters feed the common amplification report: point reads are tagged `lsm_sstable_consult`, range work is tagged `lsm_sstable_version_decoded`, data writes cover WAL records plus flush/compaction SSTable output, and primary structure is authoritative SSTable bytes. The shared Phase 4 runner exercises both engines with the same versioned point/range/sequential/random/mixed traces and archives fixed-seed raw structural evidence. Generalized levels, snapshot/replication-aware GC, device-level telemetry, controlled-host measurements, and timing-performance evidence remain deferred |
