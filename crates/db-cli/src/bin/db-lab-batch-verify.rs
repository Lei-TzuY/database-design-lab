...
struct VerificationSummary {
    ...
    failed_pairs: u32,
    excluded_pairs: u32,
    comparison_failure_sidecars: usize,
}
...
fn verify(args: &Cli) -> Result<VerificationSummary, VerifyError> {
    let index = read_json(&args.archive_dir, "index.json")?;
    ...
    if args.require_publication && !publication {
        return invalid(format!(
    ...
    let environment = read_json(&args.archive_dir, "environment.json")?;
    ...
    let sidecar_count = if contextual_failures {
        let sidecars = read_json(&args.archive_dir, "comparison-failures.json")?;
...
mod tests {
    ...
    use super::{verify, Cli, FAILURE_PROTOCOL_V2};

    #[test]
    fn verifies_v6_and_rejects_unindexed_extra_file() {
        let directory = tempdir().expect("temporary directory");
        write_archive(directory.path(), 6, false);
        let summary = verify(&args(directory.path())).expect("verify v6");
        ...
        assert!(verify(&args(directory.path()))
    ...
    fn verifies_v10_pair_join_and_rejects_tampered_pair_index() {
        ...
        let summary = verify(&args(directory.path())).expect("verify v10");
    ...
    fn frozen_v8_is_explicitly_not_strongly_verifiable() {
        ...
        let error = verify(&args(directory.path())).expect_err("v8 must fail closed");
        assert!(error.to_string().contains("without pair_index"));
    }

    fn args(path: &std::path::Path) -> Cli {
        Cli {
    ...
    fn write_archive(path: &std::path::Path, format_version: u16, with_failure: bool) {
    ...
    fn write_json(path: &std::path::Path, name: &str, value: &Value) {
        fs::write(
            path.join(name),
