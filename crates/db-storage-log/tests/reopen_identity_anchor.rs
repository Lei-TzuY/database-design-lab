...
fn failed_reopen_preserves_identity_anchor_against_later_replacement() {
    ...
    let first_error = engine.reopen().expect_err("corrupt original must fail reopen");
...

    {
        ...
        other.put(b"stable", b"replacement").expect("write replacement value");
    }
    ...
    let second_error = engine.reopen().expect_err("failed reopen must retain original identity anchor");
    assert!(second_error.to_string().contains("backing file identity changed"), "unexpected second reopen error: {second_error}");
    assert!(matches!(engine.get(b"stable"), Err(DbError::Poisoned)));
    assert_eq!(std::fs::read(&path).expect("read replacement after rejected reopen"), replacement_bytes, "identity rejection must not mutate the replacement file");
}
...
fn missing_path_reopen_preserves_identity_anchor_against_later_replacement() {
    ...
    let first_error = engine.reopen().expect_err("missing pathname must fail reopen");
    ...
    let second_error = engine.reopen().expect_err("missing-path failure must retain original identity anchor");
    assert!(second_error.to_string().contains("backing file identity changed"), "unexpected second reopen error: {second_error}");
    assert!(matches!(engine.get(b"stable"), Err(DbError::Poisoned)));
    assert_eq!(std::fs::read(&path).expect("read replacement after rejected reopen"), replacement_bytes, "identity rejection must not mutate the replacement file");
}
...
fn symlink_reopen_preserves_identity_anchor_against_later_replacement() {
    ...
    engine.reopen().expect_err("symlink pathname must fail reopen");
    assert!(matches!(engine.get(b"stable"), Err(DbError::Poisoned)));
    ...
    let second_error = engine.reopen().expect_err("symlink failure must retain original identity anchor");
    assert!(second_error.to_string().contains("backing file identity changed"), "unexpected second reopen error: {second_error}");
    assert!(matches!(engine.get(b"stable"), Err(DbError::Poisoned)));
    assert_eq!(std::fs::read(&path).expect("read replacement after rejected reopen"), replacement_bytes, "identity rejection must not mutate the replacement file");
}
